...
export function createProxyBashTool(client){
  ...
  return {
    ...
    parameters: Params,
    async execute(_id, { command, timeout }){
      // Client-level timeout is independent of bash's own  (seconds)
      // and will kill the host if it hangs to avoid wedging future calls.
      const res = await client.call('bash', { command, timeout }, { timeoutMs: BASH_TIMEOUT_MS });
      // res is { content:[{type:'text',text}], details }
      return res;
    }
  };
}
...
export function createProxyWebRenderTool(client){
  ...
  return {
    ...
    async execute(_id, args){
      ...
      return client.call('web_render', args||{}, { timeoutMs });
    }
  };
}
...
export function createProxyWebExtractTool(client){
  ...
  return {
    ...
    parameters: Params,
    async execute(_id, args){ return client.call('web_extract', args||{}, { timeoutMs: WEB_TIMEOUT_MS }); }
  };
}
...
export function createProxyWebSearchTool(client){
  ...
  return {
    ...
    parameters: Params,
    async execute(_id, args){ return client.call('web_search', args||{}, { timeoutMs: WEB_TIMEOUT_MS }); }
  };
}
